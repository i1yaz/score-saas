...
class CheckoutRepository {
    ...
    public function getPaymentData($subscription_id) {
        ...
        if (!$subscription = Subscription::on('landlord')
            ->Where('unique_id', $subscription_id)
            ->first()) {
            Log::error("unable to find the tenants subscription (unique_id: $subscription_id)", ['process' => '[checkout-payment]', config('stripe-paynow-button'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'tenant_id' => config('system.settings_saas_tenant_id')]);
            return false;
        }

        //get the package
        if (!$package = Package::on('landlord')
            ->where('id', $subscription->package_id)
            ->first()) {
            Log::error("unable to find the package for this subscription (unique_id: $subscription_id) - (package_id: " . $subscription->package_id . ")", ['process' => '[checkout-payment]', config('stripe-paynow-button'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'tenant_id' => config('system.settings_saas_tenant_id')]);
            return false;
        }

        //get landlord settings
        if (!$settings = Setting::on('landlord')
            ->Where('id', 'default')
            ->first()) {
    ...
    /**
     * The customer has paid and been redirected to the [thank you] url. This method will do all the following tasks
     *
     *  - validate the checkout session
     *  - update the customers subscription the payment gateway's new [subscription id]
    ...
    public function completeCheckoutSession($data) {
        ...
        if (!$payment_session = PaymentSession::on('landlord')->Where('gateway_ref', $data['checkout_session_id'])->first()) {
        ...
        if (!$subscription = Subscription::on('landlord')->Where('subscription_id', $payment_session->subscription_id)->first()) {
        ...
        $subscription->gateway_id = $data['gateway_subscription_id'];
        $subscription->gateway_name = $data['gateway_name'];
        $subscription->date_renewed = now();
        $subscription->status = 'active';
        $subscription->checkout_reference = $data['subscription_checkout_reference'] ?? '';
        $subscription->checkout_reference_2 = $data['subscription_checkout_reference_2'] ?? '';
        $subscription->checkout_reference_3 = $data['subscription_checkout_reference_3'] ?? '';
        $subscription->checkout_reference_4 = $data['subscription_checkout_reference_4'] ?? '';
        $subscription->checkout_reference_5 = $data['subscription_checkout_reference_5'] ?? '';
        $subscription->checkout_payload = $data['subscription_checkout_payload'] ?? '';
        $subscription->date_next_renewal = ($subscription->gateway_billing_cycle == 'monthly') ? \Carbon\Carbon::now()->addMonths(1)->format('Y-m-d') : \Carbon\Carbon::now()->addYears(1)->format('Y-m-d');
        $subscription->save();
        /** ----------------------------------------------------------------------
