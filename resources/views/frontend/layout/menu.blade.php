                    ...
                    <nav class="heading_menu">
                        <ul>
                            ...
                            <li class="heading_menu_list">
                                <a href="{{ $menu->data_2 }}"
                                    target="{{ saasLinktarget($menu->data_6) }}"
                                    class="heading_menu_list_links {{ runtimeFrontendMenuSignup($menu->data_2) }}">{{ $menu->data_1 }}</a></li>
                            @endforeach
                        </ul>
                    </nav>
                </div>
        ...
        <div class="mobile-menu-container">
            <div class="mobile-menu hidden" id="mobile-menu">
                <ul>
                    @foreach($mainmenu as $menu)
                    <li class="heading_menu_list">
                        <a href="{{ $menu->data_2 }}" target="{{ saasLinktarget($menu->data_6) }}"
                            class='heading_menu_list_links'>{{ $menu->data_1 }}</a></li>
                    @endforeach
                </ul>
            </div>
        </div>
